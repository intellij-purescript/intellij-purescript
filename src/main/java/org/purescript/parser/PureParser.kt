...
import org.purescript.psi.PSElements.Companion.Type
...
class PureParser : PsiParser, PSTokens, PSElements {
    ...
    class PureParsecParser {
        ...
        private val parseKindAtom = indented(
            choice(
                reserved("*").`as`(PSTokens.START).`as`(Star),
                reserved("!").`as`(PSTokens.BANG).`as`(Bang),
                ...
                .then(indented(
                    ...
                    .or(attempt(
                        ...
                        .then(optional(
                            ...
                            optional(indented(dcolon)).then(optional(type))
        ...
        private val parseTypeDeclaration =
            (ident.`as`(TypeAnnotationName) + dcolon + type).`as`(
                TypeDeclaration
            )
